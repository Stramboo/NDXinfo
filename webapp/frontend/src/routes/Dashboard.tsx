...
export function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* 联动：今日 NDX 大盘状态（来自 nasdaq_analyzer） */}
      <NdxStatusBar />
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card
          label="持仓市值"
          value={`$${fmtMoney(account?.market_value ?? 0)}`}
        ...
        <Card
          ...
          delta={fmtPct(account?.total_return_pct ?? 0, /*withSign*/false)}
          pos={dailyPnl >= 0}
                ...
                <div key={o.order_id} className="flex items-center justify-between text-sm border-b border-line last:border-b-0 py-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded ${
                      ...
                      o.status === "rejected" ? "bg-neg text-rose-400" :
            ...
            <div className="space-y-2">
              {signals.slice(0, 8).map((s, i) => (
                <div key={i} className="flex items-start justify-between border-b border-line last:border-b-0 py-2 text-sm">
                  ...
                  <div className="tabular text-xs text-fg-muted self-end">
                    {relTime(s.ts)}
                  </div>
                </div>
              ))}
    ...
    <div className="panel-card p-5">
      ...
      {(delta || sub) && (
        <div className="mt-1 flex items-center gap-2 text-sm">
          {delta && (
            <span className={`tabular ${pos ? "text-emerald-400" : "text-rose-400"}`}>
              {delta}
            </span>
          )}
...
function Pill({ label, active }: { label: string; active?: boolean }) {
  return (
    <button className={
      ...
      (active ? "bg-emerald-500 text-bg" : "bg-bg-subtle text-fg-muted hover:bg-bg-hover")
    }>{label}</button>
  );
}
